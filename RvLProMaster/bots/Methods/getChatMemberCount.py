  ...
  async def Initialize(cls, chat_id: Union[str, int]):
    try:
      async with ClientSession() as client:
        ...
        async with client.post(f"{api}/getChatMemberCount", data=payload) as session:
          cls.raw_json = await session.json()
          if session.status == 200:
            cls.serialize_json = json.dumps(cls.raw_json, indent=2)
            cls.status_code = session.status
          else:
            CreateLog.Error("Unable to get response from methods getChatMemberCount", str(cls.status_code))
          return cls
    except ClientConnectionError as e:
      CreateLog.Error(f"Unable to fetch from url {api}/getChatMemberCount", str(e))
      return cls